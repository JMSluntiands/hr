$(document).ready(function () {
        ...
        response.data.forEach(item => {
          table.row.add([
            ...
            userRole !== "LUNTIAN"
              ...
              : `
                <select class="form-select form-select-sm update-field" data-id="${item.job_id}" data-field="staff_id">
                  ${staffList.map(st => `<option value="${st.staff_id}" ${item.staff_id === st.staff_id ? "selected" : ""}>${st.name}</option>`).join("")}
                </select>
              `,
            ...
            userRole !== "LUNTIAN"
              ...
              : `
                <select class="form-select form-select-sm update-field" data-id="${item.job_id}" data-field="checker_id">
                  ${checkerList.map(ch => `<option value="${ch.checker_id}" ${item.checker_id === ch.checker_id ? "selected" : ""}>${ch.name}</option>`).join("")}
                </select>
              `,
            ...
            `<div class="text-center">
              ${[1, 2, 3, 4, 5].map(i => `<i class="fa fa-star ${i <= item.plan_complexity ? 'text-warning' : 'text-secondary'}"></i>`).join("")}
            </div>`,
  ...
  // 🔹 Save changes (staff/checker/status)
