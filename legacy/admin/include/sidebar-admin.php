...
if (isset($conn) && $conn instanceof mysqli) {
    if (!function_exists('incidentReportCountPending')) {
        require_once __DIR__ . '/../../database/incident_reports_schema.php';
    }
    $incidentPendingCount = incidentReportCountPending($conn);
}
$isCompensation = ($currentPage === 'compensation');
$isPerformanceReview = ($currentPage === 'performance-review');
...
?>
    ...
    <aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-40 flex max-h-[100dvh] w-64 max-w-full flex-col overflow-hidden bg-[#FA9800] text-white transform -translate-x-full transition-transform duration-200 md:translate-x-0">
        ...
        <nav class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-y-contain p-4 space-y-1 text-sm">
            ...
            <div class="dropdown-container">
                ...
                <div id="incident-report-dropdown" class="mb-2 ml-10 space-y-1<?php echo $incidentOpen; ?>" role="region" aria-label="Incident report submenu">
                    ...
                    <a href="incident-report-submitted" class="flex items-center justify-between gap-2 rounded-lg px-3 py-1.5 text-xs font-medium text-white/90 transition-colors hover:bg-white/10<?php echo $isIncidentReportSubmitted ? ' ' . $activeClass : ''; ?>">
                        <span>Incident Submitted by Employee</span>
                        <?php if ($incidentPendingCount > 0): ?>
                            <span class="min-w-[1.25rem] h-5 px-1.5 flex items-center justify-center rounded-full bg-white/90 text-[#FA9800] text-xs font-semibold"><?php echo $incidentPendingCount > 99 ? '99+' : (int) $incidentPendingCount; ?></span>
                        <?php endif; ?>
                    </a>
            ...
            <a href="performance-review" class="flex items-center gap-3 px-3 py-2 rounded-lg font-medium text-white hover:bg-white/10 transition-colors<?php echo $isPerformanceReview ? ' ' . $activeClass : ''; ?>">
        ...
        <?php include __DIR__ . '/../../include/manila-time-script.inc.php'; ?>
