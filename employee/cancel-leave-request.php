...
include '../admin/include/activity-logger.php';
...
try {
    ...
    // Also get employee name and leave details for logging
    $checkStmt = $conn->prepare("SELECT lr.id, lr.start_date, lr.status, lr.leave_type, 
                                 COALESCE(lr.days, DATEDIFF(lr.end_date, lr.start_date) + 1) as leave_days, 
                                 e.full_name 
                                 FROM leave_requests lr 
                                 JOIN employees e ON lr.employee_id = e.id 
                                 WHERE lr.id = ? AND lr.employee_id = ? AND lr.status = 'Approved'");
    if (!$checkStmt) {
        ob_clean();
        header('Content-Type: application/json');
    ...
    // Get employee name and leave details for notification
    $employeeName = $leaveRequest['full_name'] ?? 'Unknown Employee';
    $leaveType = $leaveRequest['leave_type'] ?? 'Unknown';
    $leaveDays = $leaveRequest['leave_days'] ?? $days;
    ...
    try {
        ...
        if (function_exists('logActivity')) {
            $description = "$employeeName cancelled their $leaveType request ($leaveDays day(s)). Reason: " . substr($cancelReason, 0, 100);
            logActivity($conn, 'Cancel Leave Request', 'Leave Request', $leaveId, $description);
        }
