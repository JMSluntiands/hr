...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($hasLogin) {
            $stmt2 = $conn->prepare("
                UPDATE user_logins 
                SET unique_code = ?, username = ?, client_name = 'Checker'
                WHERE unique_code = ?
            ");
        ...
        echo json_encode(['status' => 'success', 'message' => 'Checker updated successfully.']);
    } catch (Exception $e) {
        $conn->rollback();
        echo json_encode(['status' => 'error', 'message' => 'Error updating checker: ' . $e->getMessage()]);
    }
}
?>
