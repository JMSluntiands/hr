...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $conn->prepare("SELECT id FROM checker WHERE checker_id = ?");
    $check->bind_param("s", $client_code);
    $check->execute();
    $check->store_result();

    if ($check->num_rows > 0) {
        echo json_encode(['status' => 'error', 'message' => 'Checker code already exists.']);
        exit;
    }
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO checker (checker_id, name, username) VALUES (?, ?, ?)");
        ...
        echo json_encode(['status' => 'success', 'message' => 'Checker and user login added successfully.']);
    } catch (Exception $e) {
        $conn->rollback();
