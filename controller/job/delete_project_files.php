...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT job_id, upload_project_files FROM jobs WHERE job_reference_no = ?");
    ...
    $job_id       = $job['job_id'];
    $upload_files = json_decode($job['upload_project_files'], true) ?? [];
    ...
    // Kung wala nang laman, pwede gawing NULL
    $newJson = empty($newFiles) ? null : json_encode($newFiles);

    $stmt2 = $conn->prepare("UPDATE jobs SET upload_project_files = ? WHERE job_id = ?");
    $stmt2->bind_param("si", $newJson, $job_id);
    $stmt2->execute();

    // Step 3: activity log
    $desc = mysqli_real_escape_string($conn, "🗑️ Deleted Document: $filename");
    $conn->query("
