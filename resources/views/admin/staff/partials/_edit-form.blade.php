...
@php
    ...
    $resignationFileInputClass = 'block w-full text-sm text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-800';
    $stripPhonePrefix = static function ($value): string {
        $digits = (string) $value;
        return str_starts_with($digits, '09') ? substr($digits, 2) : $digits;
    };
    $phoneDisplay = $stripPhonePrefix(old('phone', $employee->phone ?? ''));
    $emergencyPhoneDisplay = $stripPhonePrefix(old('emergency_contact_phone', $employee->emergency_contact_phone ?? ''));
@endphp
<input type="hidden" name="id" value="{{ $employee->id }}">
<form method="POST" action="{{ route('admin.staff.update', $employee->id) }}" class="space-y-0" id="employeeForm" enctype="multipart/form-data" novalidate>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <textarea name="address" id="address" rows="2"
                                              ...
                                              placeholder="Primary work location / address">{{ old('address', $employee->address ?? '') }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="secondary_workplace" id="secondary_workplace" rows="2"
                                              class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow resize-none"
                                              placeholder="Optional second work location">{{ old('secondary_workplace', $employee->secondary_workplace ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                        <!-- Emergency Contact -->
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="emergency_contact_relationship" id="emergency_contact_relationship" class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow bg-white">
                                        ...
                                        <option value="Spouse" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Spouse')>Spouse</option>
                                        <option value="Partner" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Partner')>Partner</option>
                                        <option value="Parent" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Parent')>Parent</option>
                                        <option value="Grandmother" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Grandmother')>Grandmother</option>
                                        <option value="Grandfather" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Grandfather')>Grandfather</option>
                                        <option value="Sibling" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Sibling')>Sibling</option>
                                        <option value="Child" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Child')>Child</option>
                                        <option value="Friend" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Friend')>Friend</option>
                                        <option value="Other" @selected(old('emergency_contact_relationship', $employee->emergency_contact_relationship) === 'Other')>Other</option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <div class="md:col-span-3">
                                    <label class="inline-flex items-center gap-2 text-sm text-slate-700 mb-2">
                                        <input type="checkbox" id="emergencySameAsPrimary" name="emergency_same_as_primary" value="1" @if($isEmergencySameAsPrimary ?? false) checked @endif class="rounded border-slate-300 text-amber-600 focus:ring-amber-500/30">
                                    ...
                                    <div id="emergencyAddressWrap" class="mt-1">
                                        ...
                                        <textarea name="emergency_contact_address" id="emergency_contact_address" rows="2"
                                                  class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow resize-none"
                                                  placeholder="Enter emergency contact address">{{ old('emergency_contact_address', $employee->emergency_contact_address ?? '') }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="department" id="department" required class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow bg-white">
                                ...
                                @foreach($departmentOptions as $deptName)
                                    <option value="{{ $deptName }}" @selected(old('department', $employee->department) === $deptName)>{{ $deptName }}</option>
                                @endforeach
                            </select>
                            <span class="error-message text-red-500 text-xs mt-1 hidden"></span>
                        </div>
                        <div>
                            ...
                            <select name="employment_type" id="employment_type" required class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow bg-white">
                                ...
                                @foreach($employmentTypeOptions as $type)
                                    <option value="{{ $type['id'] }}" @selected((int) old('employment_type', $employee->employment_type_id ?? 0) === (int) $type['id'])>{{ $type['name'] }}</option>
                                @endforeach
                            </select>
                            <span class="error-message text-red-500 text-xs mt-1 hidden"></span>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label class="flex items-start gap-3 cursor-pointer rounded-lg border border-slate-200 bg-slate-50/80 px-4 py-3">
                                <input type="checkbox" name="performance_review_supervisor" value="1" class="mt-1 rounded border-slate-300 text-amber-600 focus:ring-amber-500" @if(old('performance_review_supervisor', $employee->performance_review_supervisor)) checked @endif>
                                <span>
                        ...
                        <div id="inactiveEmploymentFields" class="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-dashed border-slate-200 mt-2 {{ $selStatus === 'Inactive' ? '' : 'hidden' }}">
                            ...
                            <div>
                                ...
                                <input type="file" name="resignation_letter" id="resignation_letter" accept=".pdf,.jpg,.jpeg,.png"
                                       class="{{ $resignationFileInputClass }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nbi_clearance" id="nbi_clearance" 
                                   value="{{ old('nbi_clearance', $employee->nbi_clearance ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="police_clearance" id="police_clearance" 
                                   value="{{ old('police_clearance', $employee->police_clearance ?? '') }}"
                                   placeholder="Clearance number or reference"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="basic_salary_daily" id="basic_salary_daily"
                                   value="{{ old('basic_salary_daily', $employeeCompensation->basic_salary_daily ?? 0) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="allowance_internet" id="allowance_internet"
                                   value="{{ old('allowance_internet', $employeeCompensation->allowance_internet ?? 0) }}"
                                   min="0" step="0.01" placeholder="0.00"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow">
                        </div>
                        <div>
                            ...
                            <input type="number" name="allowance_meal" id="allowance_meal"
                                   value="{{ old('allowance_meal', $employeeCompensation->allowance_meal ?? 0) }}"
                                   min="0" step="0.01" placeholder="0.00"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow">
                        </div>
                        <div>
                            ...
                            <input type="number" name="allowance_position" id="allowance_position"
                                   value="{{ old('allowance_position', $employeeCompensation->allowance_position ?? 0) }}"
                                   min="0" step="0.01" placeholder="0.00"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow">
                        </div>
                        <div>
                            ...
                            <input type="number" name="allowance_transportation" id="allowance_transportation"
                                   value="{{ old('allowance_transportation', $employeeCompensation->allowance_transportation ?? 0) }}"
                                   min="0" step="0.01" placeholder="0.00"
                                   class="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow">
                        </div>
                        <div>
                ...
                <div class="p-6 md:p-8 flex flex-col sm:flex-row justify-end gap-3 bg-slate-50/80 border-t border-slate-200">
                    ...
                    <x-hr-btn type="submit">Update Employee</x-hr-btn>
                </div>
            </form>